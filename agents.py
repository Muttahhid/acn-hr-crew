    ...
    def DigitalAgent(self, candidateProfile):
        ...
        return Agent(
            ...
            backstory=dedent(f"""
                ...
                Use the AcnPDFReader tool in preference to fetch information from {candidateProfile}
            """),
            goal=dedent(f"""
                Remember to focus on accuracy, relevancy, and ensuring that the extracted content aligns with the given parameters. Ensure that the content is presented in a clear and concise manner for easy comprehension.
            """),
            # tools=[tool_1, tool_2],
            allow_delegation=False,
            verbose=True,
            llm=self.Groq,
            tool=[PDFSearchTool(pdf=candidateProfile)]
        )
